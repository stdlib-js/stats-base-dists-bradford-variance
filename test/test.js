...
tape( 'the function returns the variance of a Bradford distribution given small parameter `c`', function test( t ) {
	var expected;
	var i;
	var c;
	var y;
	...
	*/
	for ( i = 0; i < expected.length; i++ ) {
		y = variance( c[i] );
		t.strictEqual( isAlmostSameValue( y, expected[ i ], 856 ), true, 'returns expected value' );
	}
	t.end();
});

tape( 'the function returns the variance of a Bradford distribution given large parameter `c`', function test( t ) {
	var expected;
	var i;
	var c;
	var y;
